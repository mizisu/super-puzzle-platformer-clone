...
BackgroundLayer::BackgroundLayer() {
  auto background = std::make_shared<Sprite>("play/background.png");
...
void BackgroundLayer::CreateBottomLine() {
  for (int i = 0; i < 10; i++) {
    auto sp = std::make_shared<Sprite>("play/block/bottom.png");
...
void BackgroundLayer::CreatePill() {
  auto texturepath = "play/block/pill.png";
  for (int i = 0; i < 11; i++) {
...
void BackgroundLayer::CreateEdge() {
  auto lefttop = std::make_shared<Sprite>("play/block/edge.png");
  lefttop->SetPosition(0, UIHeihgt);
  this->AddChild(lefttop);

  auto righttop = std::make_shared<Sprite>("play/block/edge.png");
  righttop->SetPosition(background_width - righttop->Width(), UIHeihgt);
  this->AddChild(righttop);

  auto leftbottom = std::make_shared<Sprite>("play/block/edge.png");
  leftbottom->SetPosition(0, background_height - leftbottom->Height());
  this->AddChild(leftbottom);

  auto rightbottom = std::make_shared<Sprite>("play/block/edge.png");
